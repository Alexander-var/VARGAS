...
$nomdia = array("Domingo", "Lunes", "Martes", "Miercoles", "Jueves",
"Viernes", "Sabado");
$dia = date("j");
$mes = date("n");
$diasemana = date("w");
$hoy = $nomdia[$diasemana]." ".$dia." de ".$nommes[$mes-1]."
".date("Y")."";
$mes=$_GET['mes'];
$anio=$_GET['anio'];
...
?>
<h3 cellpadding="0" cellspacing="2" width="200" border="0"
style="text-align: center; display:block;">
 <?php echo '<th bgstyle="color:#4069B1" scope="col">'.$nommes[$mes].' '.$anio.'</th>';?>
</h3>
<br>
<table cellpadding="0" cellspacing="2" width="200" border="0"
style="text-align: center;">
 <tr height="17">
 <?php foreach($spDays AS $dd) echo '<th bgstyle="color:#4069B1"
scope="col">'.$dd.'</th>';?>
 </tr>
 <?php
if ($v < 7) {
?><tr><?php
foreach ($fDays2Use AS $k => $v) echo '<td
bgstyle="color:#F0F5FF">'.$v.'</td>';
?><tr><?php
}
reset($days2Use);
for ($x = 1; $x <= $top;$x++) {
...
?><tr><?php
for ($p = 1; $p <= 7;$p++) {
?><td bgstyle="color:#<?php echo $bgcolor?>"><?php echo
(current($days2Use)) ? current($days2Use) : "&nbsp;"?></td><?php
next($days2Use);
}
?></tr><?php
} 
